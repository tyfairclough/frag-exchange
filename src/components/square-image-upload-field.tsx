...
type SquareImageUploadFieldProps = {
  inputName: string;
...
async function renderSquareBlob(params: {
  ...
  const outSize = params.outputSize;
  ...
  return new Promise((resolve) => {
    canvas.toBlob((blob) => resolve(blob), params.mimeType, params.outputQuality);
  });
}

export function SquareImageUploadField({
  inputName,
  ...
  const [fileName, setFileName] = useState<string>(outputFileName);
  ...
  useEffect(() => {
    ...
    async function assignCroppedFile() {
      ...
      dt.items.add(new File([blob], fileName || outputFileName, { type: mimeType }));
  ...
  }, [draftObjectUrl, fileName, mimeType, outputFileName, outputQuality, outputSize, sourceUrl, xOffset, yOffset, zoom]);

  return (
    <fieldset className={`space-y-3 rounded-xl border border-slate-200 bg-slate-50/60 p-3 ${className ?? ""}`}>
      <legend className="px-1 text-sm font-semibold text-slate-700">{label}</legend>
      <input ref={submitFileRef} name={inputName} type="file" className="hidden" tabIndex={-1} />
      ...
      <div className="flex flex-wrap items-start gap-3">
        <div className="relative h-28 w-28 overflow-hidden rounded-xl border border-slate-300 bg-white">
          {sourceUrl ? (
            ...
            <div className="flex h-full w-full items-center justify-center text-xs text-slate-500">{emptyLabel}</div>
          )}
        </div>
        <div className="min-w-[220px] flex-1 space-y-2">
          <input
            ...
            onChange={(event) => {
              ...
              setFileName(next.name || outputFileName);
              setZoom(1);
              setXOffset(0);
              setYOffset(0);
            }}
