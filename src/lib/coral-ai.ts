      ...
      messages: [
        {
          role: "system",
          content:
            'You classify reef-hobby listings from photos. Reply JSON only: {"itemKind":"CORAL"|"FISH"|"EQUIPMENT"|null, "name":string|null (max 120 chars), "description":string (2-4 sentences), "coralType":string|null (Soft|LPS|SPS for CORAL), "colour":string|null (short; for CORAL/FISH), "species":string|null (FISH), "reefSafe":boolean|null (FISH: true if reef-safe), "equipmentCategory":string|null (EQUIPMENT: one of Lights|Pumps|Monitors & Controllers|Filtration|Dosing|Other), "equipmentCondition":string|null (EQUIPMENT: one of Like new|Good condition|Working|Spares & Repairs)}. Use null when unsure. If the photo is not aquarium-related, still pick the closest kind.',
        },
        {
          role: "user",
          content: [
            { type: "text", text: "Classify this listing photo and fill fields for a swap marketplace." },
